...
def calculate_segmentation_point(
            ...
            for tensor in tensor_data:
                ...
                #     print(f"Warning: Object {obj_id} is higher than camera.")
                    ...
                    for whiteline_data in whiteline_datas:
                        wl_id = whiteline_data["ID"]
                        m = whiteline_data["Coefficient_m"]
                        c = whiteline_data["Coefficient_c"]

                        whiteline_pixel_x = int(m * y + c)

                        if proj_mode == 0:
                            whiteline_results = calc_relative_coord_ctr_proj(
                                image_path, theta, y, whiteline_pixel_x, camera_elevation_angle, camera_height, True
                            )
                        elif proj_mode == 1:
                            whiteline_results = calc_relative_coord_equidistant_proj(
                                image_path, theta, y, whiteline_pixel_x, camera_elevation_angle, camera_height, True
                            )

                        whiteline_relative_coordinates = whiteline_results[0]
                        whiteline_dx = round(whiteline_relative_coordinates[0], ROUNDED_DIGIT_NUM)
                        whiteline_dy = round(whiteline_relative_coordinates[1], ROUNDED_DIGIT_NUM)
                        obj_distance_from_whiteline = {
                            "whiteline_id": wl_id,
                            "whiteline_dx": whiteline_dx,
                            "whiteline_dy": whiteline_dy
                        }
                        whiteline_distances.append(obj_distance_from_whiteline)
                    ...
                    calculate_result_elem = {
                        ...
                        "whiteline_distance": whiteline_distances
                    }
    ...
    json_file_name = os.path.join(
        output_dir, "segmentation_detection_result_with_lane.json")
