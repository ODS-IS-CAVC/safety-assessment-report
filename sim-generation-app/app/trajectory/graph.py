...
def do_plot(i, out_dir, f, t, gx, gx_ma, lat, lon, spd, spd_ma):
    ...
    if i >= 0 and not np.isnan(lat[i]):
        ...
        plt.text(pos[0], pos[1], txt, fontname='Yu Gothic', fontsize=11)
        #plt.text(pos[0], pos[1], txt, fontsize=8)
    plt.xlabel('Longitude', fontname='Yu Gothic', fontsize=18)
    plt.ylabel('Latitude', fontname='Yu Gothic', fontsize=18)
    plt.title('Coordinate', fontname='Yu Gothic', fontsize=18)
    ...
    if i >= 0 and not np.isnan(gx_ma[i]):
        ...
        plt.text(pos[0], pos[1], txt, fontname=font_name, fontsize=11)
        #plt.text(pos[0], pos[1], txt, fontsize=8)
    plt.xlabel('Time (s)', fontname=font_name, fontsize=18)
    plt.ylabel('G', fontname=font_name, fontsize=18)
    plt.title('G X', fontname=font_name, fontsize=18)
    ...
    if i >= 0 and not np.isnan(spd_ma[i]):
        ...
        #plt.text(pos[0], pos[1], txt, fontsize=8)
    plt.xlabel('Time (s)', fontname=font_name, fontsize=18)
    plt.ylabel('Speed (km/h)', fontname=font_name, fontsize=18)
    plt.title('Speed', fontname=font_name, fontsize=18)
    #plt.xlabel('Time (s)', fontsize=14)
